        ...
        <div className="container">
          <div className={styles.storyGrid}>
            <div>
              ...
              <h2 style={{ fontSize: "2.2rem", fontWeight: 700, color: "var(--dark)", marginBottom: 36, maxWidth: 480, lineHeight: 1.2 }}>
                Nurturing learning and play since 2011
              </h2>
              <p style={{ fontSize: "1.05rem", color: "var(--text-light)", lineHeight: 1.85, marginBottom: 20 }}>
                Playhouse Nursery was founded with a single mission: to create a warm, secure, and inspiring second home where children can explore their natural curiosity. Over the past decade, we have grown into one of the UAE&apos;s most trusted nursery groups, approved by both KHDA and ADEK.
              </p>
              <p style={{ fontSize: "1.05rem", color: "var(--text-light)", lineHeight: 1.85, marginBottom: 20 }}>
            ...
            <div className={styles.factsGrid}>
              {[
                ...
                { num: "KHDA & ADEK", label: "Approved" },
                { num: "15", label: "Max Class Size" },
                { num: "4", label: "Meals Per Day" },
                { num: "EYFS", label: "British Curriculum" },
              ].map((s) => (
                <div key={s.label} style={{ background: "var(--white)", padding: "36px 24px", textAlign: "center", display: "flex", flexDirection: "column", justifyContent: "center" }}>
                  <span style={{ display: "block", fontSize: s.num.includes("&") ? "1.5rem" : "2.4rem", fontWeight: 700, color: "var(--dark)", lineHeight: 1.2, marginBottom: s.num.includes("&") ? 4 : 10 }}>{s.num}</span>
                  <span style={{ display: "block", fontSize: s.num.includes("&") ? "1.5rem" : "0.82rem", fontWeight: s.num.includes("&") ? 700 : 600, color: s.num.includes("&") ? "var(--dark)" : "var(--text-light)" }}>{s.label}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      <section id="awards" style={{ padding: "100px 0", background: "var(--dark)" }}>
        <div className="container">
          ...
          <h2 style={{ fontSize: "2.2rem", fontWeight: 700, color: "#fff", marginBottom: 52, lineHeight: 1.2 }}>
            ADEK and KHDA Approved Nursery
          </h2>
          <div className={styles.accreditationsGrid}>
            {[
              ...
              <div key={a.title} className={styles.accreditationCard}>
